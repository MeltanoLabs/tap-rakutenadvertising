# AGENTS.md - AI Agent Development Guide for tap-rakutenadvertising
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from RakutenAdvertising.

### Key Components

1. **Tap Class** (`tap_rakutenadvertising/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_rakutenadvertising/client.py`): Handles API communication and authentication
1. **Streams** (`tap_rakutenadvertising/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_rakutenadvertising/streams.py`
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(RakutenAdvertisingStream):
...
```bash
tap-rakutenadvertising --config config.json --discover
...
1. Update `config_jsonschema` in `tap_rakutenadvertising/tap.py`
...
```bash
# .env.example
TAP_RAKUTENADVERTISING_API_URL=https://api.example.com
TAP_RAKUTENADVERTISING_API_KEY=your_api_key_here
TAP_RAKUTENADVERTISING_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-rakutenadvertising --version`), Python version, and a minimal reproduction case when reporting bugs.
